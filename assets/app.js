    ...
    fetch('/api/youtube-shorts')
      ...
      .then(function(data){
        ...
        data.shorts.forEach(function(short){
          ...
          // hqdefault (480x360) looks soft/pixelated once stretched to
          // fill a tall 9:16 card at real card width — try the sharper
          // maxresdefault first (1280x720, not always generated by
          // YouTube for every upload) and silently fall back to the
          // known-good hqdefault URL the API already gave us if it
          // 404s.
          var img = document.createElement('img');
          img.src = short.thumbnail.replace('/hqdefault.jpg', '/maxresdefault.jpg');
          img.alt = '';
          img.loading = 'lazy';
          img.onerror = function(){ img.onerror = null; img.src = short.thumbnail; };
